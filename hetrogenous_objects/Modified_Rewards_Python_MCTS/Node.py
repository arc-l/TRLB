...
from Modified_Rewards_Python_MCTS.QuadTree import QuadTree, Item
...
class Node(object):
    def __init__(
        ...
        self.visited_time = 0
        self.needed_efforts = 0 # neede effort to come here
        self.total_reward = 0
        self.radius = radius
        self.Width = Width
        self.Height = Height
    ...
    def UCB(self, use_heuristic):
        ...
        rescaling = 1.0/min([p[6] for p in self.current_arr.values()])
        for action, child in self.children.items():
            if use_heuristic == 0:
                Reward = child.total_reward/child.visited_time + self.UCB_scalar * \
                    sqrt(2*log1p(self.visited_time)/child.visited_time)
            elif use_heuristic == 1:
                Reward = child.total_reward/child.visited_time + self.UCB_scalar * \
                    sqrt(2*log1p(self.visited_time)/child.visited_time) -\
                    self.current_arr[action][6]*rescaling
