...
def fig_high_level_05():
    ...
    ax.bar(np.array(Selected_NumObjs)-1.5*bar_width, [Avg_Results[Compared_Methods[0]][Selected_Densities[0]][numObjs]['Time'] for numObjs in Selected_NumObjs], bar_width, **alg1Style)
    ...
    ax.bar(np.array(Selected_NumObjs)+0.5*bar_width, [Avg_Results[Compared_Methods[2]][Selected_Densities[0]][numObjs]['Time'] for numObjs in Selected_NumObjs], bar_width, **alg3Style)
    ax.bar(np.array(Selected_NumObjs)+1.5*bar_width, [Avg_Results[Compared_Methods[3]][Selected_Densities[0]][numObjs]['Time'] for numObjs in Selected_NumObjs], bar_width, **alg4Style)

    ax.set_xlim([Selected_NumObjs[0]-2.2 * bar_width, 
    Selected_NumObjs[-1]+2.2 * bar_width])
    ...
    ax2.bar(np.array(Selected_NumObjs)-1.5*bar_width, [Avg_Results[Compared_Methods[0]][Selected_Densities[0]][numObjs]['Rate'] for numObjs in Selected_NumObjs], bar_width, **alg1Style)
    ...
    ax2.bar(np.array(Selected_NumObjs)+0.5*bar_width, [Avg_Results[Compared_Methods[2]][Selected_Densities[0]][numObjs]['Rate'] for numObjs in Selected_NumObjs], bar_width, **alg3Style)
    ax2.bar(np.array(Selected_NumObjs)+1.5*bar_width, [Avg_Results[Compared_Methods[3]][Selected_Densities[0]][numObjs]['Rate'] for numObjs in Selected_NumObjs], bar_width, **alg4Style)

    ax2.set_xlim([Selected_NumObjs[0]-2.2 * bar_width, 
    Selected_NumObjs[-1]+2.2 * bar_width])
    ...
    ax2.bar(np.array(Selected_NumObjs)-1.5*bar_width, [Avg_Results[Compared_Methods[0]][Selected_Densities[0]][numObjs]['Ratio'] for numObjs in Selected_NumObjs], bar_width, **alg1Style)
    ...
    ax2.bar(np.array(Selected_NumObjs)+0.5*bar_width, [Avg_Results[Compared_Methods[2]][Selected_Densities[0]][numObjs]['Ratio'] for numObjs in Selected_NumObjs], bar_width, **alg3Style)
    ax2.bar(np.array(Selected_NumObjs)+1.5*bar_width, [Avg_Results[Compared_Methods[3]][Selected_Densities[0]][numObjs]['Ratio'] for numObjs in Selected_NumObjs], bar_width, **alg4Style)
    ...
    fig.savefig( os.path.dirname(my_path) + "/figures/high_level_05.eps", bbox_inches="tight", pad_inches=0.05)
    # plt.title("Density="+str(Density))
    plt.show()
    # plt.cla()


def fig_high_level_03():
    Compared_Methods = [
        "OS_SP_RB", 
        "ST_SP_RB", 
        "BST_SP_RB", 
    ...
    # Directly save the figure to a file.
    fig.savefig( os.path.dirname(my_path) + "/figures/high_level_03.eps", bbox_inches="tight", pad_inches=0.05)
    # plt.title("Density="+str(Density))
    plt.show()
    # plt.cla()
...
def fig_dense_small():
    ...
    # Here, we setup the style for an algorithm. Let's call it Alg.1.
    alg1Style = deepcopy(defaultStyle)          # First copy all default styles.
    alg1Style['label'] = r'\textsc{RBM-SP-ST}'    # Setup algorithm name. 
                                                # We use \textsc here which is a latex command. 
                                                # This is fine since we use latex to generate text.
    ...
    if len(Compared_Methods)>=2:
        # Another algorithm
        alg2Style = deepcopy(defaultStyle)
        alg2Style['label'] = r'\textsc{RBM-SP-BST}'
        alg2Style['color'] = 'tab:orange'

    if len(Compared_Methods)>=3:
        # Another algorithm
        alg3Style = deepcopy(defaultStyle)
        alg3Style['label'] = r'BiRRT(fmRS)'
        alg3Style['color'] = 'tab:orange'
    ...
    ''' Some global variables '''
    FIGURE_SIZE = (6, 6)      # Figure width and height. 
                                    # This is a good value for 2-column paper.
    FONT_SIZE = 36                   # Size of text
    LEGEND_FONT_SIZE = 24            # We might need different font sizes for different text
    ...
    # ax2.set_ylim([1e-4, 1e3])
    # Set x and y label. We use latex to generate text
    ax2.legend(fontsize = LEGEND_FONT_SIZE, ncol = 1, loc='upper left')
    ax2.set_xticks(Selected_NumObjs)
    ...
    # Directly save the figure to a file.
    fig.savefig( os.path.dirname(my_path) + "/figures/ST_BST_dense_small.eps", bbox_inches="tight", pad_inches=0.05)
    # plt.title("Density="+str(Density))
    plt.show()
    # plt.cla()
