...
def fig_large_scale():
    ...
    Selected_NumObjs = [ 20, 40, 60, 80, 100]
    ...
    alg1Style['label'] = r'\textsc{RBM-SP-BST}'    # Setup algorithm name. 
    ...
    ax.bar(np.array(Selected_NumObjs)-1.0*bar_width, [Avg_Results[Compared_Methods[0]][Selected_Densities[0]][numObjs]['Time'] for numObjs in Selected_NumObjs], bar_width, **alg1Style)
    ax.bar(np.array(Selected_NumObjs)-0.0*bar_width, [Avg_Results[Compared_Methods[1]][Selected_Densities[0]][numObjs]['Time'] for numObjs in Selected_NumObjs], bar_width, **alg2Style)
    ...
    ax2.bar(np.array(Selected_NumObjs)-1.0*bar_width, [Avg_Results[Compared_Methods[0]][Selected_Densities[0]][numObjs]['Coll'] for numObjs in Selected_NumObjs], bar_width, **alg1Style)
    ax2.bar(np.array(Selected_NumObjs)-0.0*bar_width, [Avg_Results[Compared_Methods[1]][Selected_Densities[0]][numObjs]['Coll'] for numObjs in Selected_NumObjs], bar_width, **alg2Style)
    ax2.bar(np.array(Selected_NumObjs)+1.0*bar_width, [Avg_Results[Compared_Methods[2]][Selected_Densities[0]][numObjs]['Coll'] for numObjs in Selected_NumObjs], bar_width, **alg3Style)
    ...
    ax2.bar(np.array(Selected_NumObjs)-1.0*bar_width, [Avg_Results[Compared_Methods[0]][Selected_Densities[0]][numObjs]['Ratio'] for numObjs in Selected_NumObjs], bar_width, **alg1Style)
    ax2.bar(np.array(Selected_NumObjs)-0.0*bar_width, [Avg_Results[Compared_Methods[1]][Selected_Densities[0]][numObjs]['Ratio'] for numObjs in Selected_NumObjs], bar_width, **alg2Style)
    ...
    fig.savefig( os.path.dirname(my_path) + "/figures/large_scale.eps", bbox_inches="tight", pad_inches=0.05)
...
def fig_dense_small():
    ...
    ax2.bar(np.array(Selected_NumObjs)-1.0*bar_width, [Avg_Results[Compared_Methods[0]][Selected_Densities[0]][numObjs]['Rate'] for numObjs in Selected_NumObjs], bar_width, **alg1Style)
    ax2.bar(np.array(Selected_NumObjs)-0.0*bar_width, [Avg_Results[Compared_Methods[1]][Selected_Densities[0]][numObjs]['Rate'] for numObjs in Selected_NumObjs], bar_width, **alg2Style)
    ...
    # Directly save the figure to a file.
    fig.savefig( os.path.dirname(my_path) + "/figures/dense_small.eps", bbox_inches="tight", pad_inches=0.05)
    # plt.title("Density="+str(Density))
    plt.show()
    # plt.cla()
...
def fig_stick():
    ...
    if len(Compared_Methods)>=2:
        # Another algorithm
        alg2Style = deepcopy(defaultStyle)
        alg2Style['label'] = r'\textsc{BiRRT(fmRS)}'
        alg2Style['color'] = 'tab:orange'

    if len(Compared_Methods)>=3:
        # Another algorithm
        alg3Style = deepcopy(defaultStyle)
        alg3Style['label'] = r'\textsc{RO-SP-BST}'
        alg3Style['color'] = 'tab:orange'
    ...
    ax.set_xlim([Selected_NumObjs[0]-1.0 * bar_width, 
    Selected_NumObjs[-1]+1.0 * bar_width])
    ...
    ax2.set_xlim([Selected_NumObjs[0]-1.0 * bar_width, 
    Selected_NumObjs[-1]+1.0 * bar_width])
    # ax2.set_ylim([1e-4, 1e3])
    # Set x and y label. We use latex to generate text
    # ax.legend(fontsize = LEGEND_FONT_SIZE, ncol = 1)
    ...
    # Directly save the figure to a file.
    fig.savefig( os.path.dirname(my_path) + "/figures/stick_03.eps", bbox_inches="tight", pad_inches=0.05)
    # plt.title("Density="+str(Density))
    plt.show()
    # plt.cla()
