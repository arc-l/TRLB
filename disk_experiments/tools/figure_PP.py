...
def fig_PP_05():
    ...
    fig.savefig( os.path.dirname(my_path) + "/figures/PP_05.eps", bbox_inches="tight", pad_inches=0.05)
...
def fig_PP_03():
    ...
    if len(Compared_Methods)>=2:
        # Another algorithm
        alg2Style = deepcopy(defaultStyle)
        alg2Style['label'] = r'\textsc{RBM-SP-OS-PP}'
        alg2Style['color'] = 'tab:green'
    ...
    # Directly save the figure to a file.
    fig.savefig( os.path.dirname(my_path) + "/figures/PP_03.eps", bbox_inches="tight", pad_inches=0.05)
    # plt.title("Density="+str(Density))
    plt.show()
    # plt.cla()
...
def fig_dense_small():
    ...
    ''' Some global variables '''
    FIGURE_SIZE = (6, 4)      # Figure width and height. 
                                    # This is a good value for 2-column paper.
    FONT_SIZE = 36                   # Size of text
    LEGEND_FONT_SIZE = 24            # We might need different font sizes for different text
    ...
    # ax2.set_ylim([1e-4, 1e3])
    # Set x and y label. We use latex to generate text
    ax2.legend(fontsize = LEGEND_FONT_SIZE, ncol = 1)
    ax2.set_xticks(Selected_NumObjs)
