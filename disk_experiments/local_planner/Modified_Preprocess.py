...
class Unlabeled_Partial_Optimizer(object):
    def __init__(self, start_arr, goal_arr, SCC, Height, Width, radius, sampling=False):
        ...
        for obj in SCC:
            # check whether they are in the interested SCC, the goal positions are assumed to be independent to the other objects
    ...
    def construct_matchup(self, goal_pose_ordering, Bi_DG, remain_start_poses):
        ...
        for goal in goal_pose_ordering:
            moving_objects = []
            for start in Bi_DG[goal]:# potential collisions
                if start in remain_start_poses:
                    moving_objects.append(start)
                    remain_start_poses.remove(start)
            if (len(moving_objects)==0): # collision free
                if (len(current_buffer)>0): # if some objs are at the buffer
    ...
    def call_subproblems(self, action_sequence, Problem_Partition, start_arr, goal_arr):
        ...
        action_list = []
        isfeasible = True
        current_arr = {k:p for k,p in start_arr.items()}
