...
class Buffer_Sampling(object):
    ...
    def buffer_sampling(self, buffers, buffer_current_location_dict, obs_dict):
        ...
        while (timeout > 0) and (not isfeasible):
            ...
            for bufferID in buffers:
                ...
                if not self.stick_collision_check(buffer_current_location_dict[bufferID], obs_list):
                    previous_buffer_feasible = True 
                    new_buffer_dict[bufferID] = buffer_current_location_dict[bufferID]
                    continue
    ...
    def stick_collision_check(self, obj, obs_list):
        isColliding = False
        obj_poly = array(
            poly_stick(
                [obj[0], obj[1]], obj[2], self.Length, 
                self.ratio*self.Length
            )
        )
        for obs in obs_list:
            obs_poly = array(
                poly_stick(
                    [obs[0], obs[1]], obs[2], self.Length, 
                    self.ratio*self.Length
                )
            )
