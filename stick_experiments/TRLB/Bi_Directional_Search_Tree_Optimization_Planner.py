...
class Bi_Directional_Search_Tree_Planner(object):
    ...
    '''
    def __init__(self, start_arr, goal_arr, Height, Width, Density, WL_ratio = 0.3, sampling=False, primitive_plan = 'RB', robust_sampling = False, PP = False):
    ...
    def growSubTree(self, startID, goalID, side='Left'):
        ...
        if side == 'Left':
            ...
            partial_planner = Labeled_Partial_Optimizer(start_arr, goal_arr, self.Height, self.Width, self.Length, self.ratio)
    ...
    def construct_plan(self):
        ...
        for ii, action in enumerate(self.action_list[:-1]):
            p1 = action[1]
            p2 = action[2]
            if (norm(array(p1)-array(p2))<Epsilon):
